...
extern UINT16             SP1_G_Addr;
extern EFI_SERIAL_IO_MODE mSerialIoMode;
...
{
	UINT8  State = 0;
	UINTN  Result = NumberOfBytes;
	
	while (NumberOfBytes != 0) {
		
		while( !( State & (1<<LSR_TXEMP_BIT | 1<<LSR_TXRDY_BIT) ) )
		{
			//ReadMem(EfiCpuIoWidthUint8, (*SP1_Adr + LSR_REG), 1, &Buffer);
			State = IoRead8 (SP1_G_Addr + LSR_REG);
		}
		
		//WriteMem(EfiCpuIoWidthUint8, *SP1_Adr, 1, Input);
  ...
  )
{
	...
	for (Result = 0; NumberOfBytes-- != 0; Result++, Buffer++) {
		...
		while( !( State & (1<<LSR_RXDA_BIT) ) )
		{
			//ReadMem(EfiCpuIoWidthUint8, (*SP1_Adr + LSR_REG), 1, &Buffer);
			State = IoRead8(SP1_G_Addr + LSR_REG);
			if( 0x02 == ( State & (1<<LSR_OE_BIT) ) )
			{
				//Print(L"RX FIFO overrun\r\n");
			}
		}
	
		//buf[Count++] = IoRead8 ((UINTN) *SP1_Adr);
...
{
	UINT8  State = State = IoRead8(SP1_G_Addr + LSR_REG);
	if( ( State & (1<<LSR_RXDA_BIT) ) )
	{
		return TRUE;
	}
  ...
  )
{
  ...
  WRITE_LCR (SerialDevice, Lcr.Data);
  //Print(L"Lcr.Bits.SerialDB: 0x%x\r\n",Lcr.Bits.SerialDB);
